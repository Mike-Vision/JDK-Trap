...
public class RobloxFullscreen
{
    ...
    public static void WaitAndTriggerFullscreen()
    {
        ...
        string processName = JDKTrap.App.RobloxPlayerAppName.Split('.')[0];
        JDKTrap.App.Logger.WriteLine(LOG_IDENT, $"Waiting for {processName} to start and become visible...");
        ...
        while (sw.Elapsed.TotalSeconds < 60)
        {
            ...
            try
            {
                if (processes.Length > 0)
                {
                    ...
                    {
                        JDKTrap.App.Logger.WriteLine(LOG_IDENT, "Found visible Roblox window, triggering Alt+Enter");

                        SetForegroundWindow(roblox.MainWindowHandle);
                        Thread.Sleep(500);

                        SendAltEnter();
                        JDKTrap.App.Logger.WriteLine(LOG_IDENT, "Alt+Enter triggered");
                        return;
                    }
                }
            }
        ...
        JDKTrap.App.Logger.WriteLine(LOG_IDENT, "Timed out waiting for Roblox window");
    }
