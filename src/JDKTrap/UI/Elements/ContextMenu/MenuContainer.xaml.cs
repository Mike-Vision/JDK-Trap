...
using JDKTrap.UI.Chat;
using JDKTrap.UI.Elements.Crosshair;
using JDKTrap.UI.Elements.FPS;
using JDKTrap.UI.Elements.Overlay;
...
namespace JDKTrap.UI.Elements.ContextMenu
{
    ...
    public partial class MenuContainer
    {
        ...
        private void MemoryTimer_Tick(object? sender, EventArgs e)
        {
            try
            {
                var jdktrapProcess = Process.GetProcessesByName("JDKTrap").FirstOrDefault();
                long jdktrapMemory = jdktrapProcess?.WorkingSet64 ?? 0;
        ...
        public void ActivityWatcher_OnGameJoin(object? sender, EventArgs e)
        {
            ...
            if (App.Settings.Prop.IngameChatDiscord)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    if (App.Current.Resources["DiscordChatOverlayWindow"]
                        is JDKTrap.UI.Elements.Overlay.DiscordChatOverlayWindow existing)
                    {
                    ...
                    var discordOverlay = new JDKTrap.UI.Elements.Overlay.DiscordChatOverlayWindow();
                    discordOverlay.Show();
            ...
            if (App.Settings.Prop.OverlaysEnabled)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    if (App.Current.Resources["OverlayWindow"] is JDKTrap.UI.Elements.Overlay.OverlayWindow existing)
                    {
                        if (existing.IsLoaded)
                        {
                            return;
                    ...
                    var overlay = new JDKTrap.UI.Elements.Overlay.OverlayWindow();
                    overlay.Show();
        ...
        public void ActivityWatcher_OnGameLeave(object? sender, EventArgs e)
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                ...
                if (App.Current.Resources["OverlayWindow"] is JDKTrap.UI.Elements.Overlay.OverlayWindow overlay)
                {
                    overlay.Close();
                    App.Current.Resources.Remove("OverlayWindow");
                }

                if (App.Current.Resources["DiscordChatOverlayWindow"] is JDKTrap.UI.Elements.Overlay.DiscordChatOverlayWindow discordOverlay)
                {
