...
using static JDKTrapRobloxSettingsManager;


namespace JDKTrap.UI.ViewModels.Settings
{
    public class BehaviourViewModel : NotifyPropertyChangedViewModel
    {
        ...
        {
            var settings = new JDKTrapRobloxSettings
            {
                MemoryCleanerIntervalSeconds =
                    SelectedMemoryCleanerInterval?.Seconds ?? 0
            };

            JDKTrapRobloxSettingsManager.Save(settings);
        }
        ...
        public bool CleanerJDKTrap
        {
            get => CleanerItems.Contains("JDKTrapLogs");
            set
            {
                if (value && !CleanerItems.Contains("JDKTrapLogs"))
                {
                    CleanerItems.Add("JDKTrapLogs");
                    UpdateCleanerItems();
                }
                else if (!value && CleanerItems.Contains("JDKTrapLogs"))
                {
                    CleanerItems.Remove("JDKTrapLogs");
                    UpdateCleanerItems();
                }
                OnPropertyChanged(nameof(CleanerJDKTrap));
            }
        }
