    ...
    Task.Run(() =>
    {
        ...
        private void UpdateRpcPresence(bool force = false)
        {
            ...
            if (NowPlaying == null || string.IsNullOrWhiteSpace(NowPlaying.Title))
            {
                ...
                {
                    Details = "Idle",
                    State = "JDKTrap Music Player",
                    Assets = new DRPC.Assets
                    {
                        LargeImageKey = "jdktrap_logo",
                        LargeImageText = "JDKTrap Music Player"
                    }
                };
            }
            else
            {
                ...
                string infoString = $"{(NowPlaying.FileType ?? "FILE").ToUpperInvariant()} • {elapsedStr} / {totalStr} • {(_isPlaying ? "Playing" : "Paused")} in JDKTrap{loopText}";
