...
namespace RobloxDX12Optimizer
{
    public static class RobloxDx12Optimizer
    {
        ...
        private static bool _isVendorProbed = false;
        ...
        {
            if (_isVendorProbed) return;
            _isVendorProbed = true;
            Task.Run(() =>
            {
                try
                {
        ...
        private static void ProbeForDx12UsageSafe(Process proc)
        {
            Task.Run(() =>
            {
                try
                {
                    ...
                    try
                    {
                        hasDx12 = proc.Modules.Cast<ProcessModule>().Any(m => 
                            m.ModuleName != null && m.ModuleName.Equals("d3d12.dll", StringComparison.OrdinalIgnoreCase));
                    }
                    catch
                    {
                        // Thỉnh thoảng Modules ném lỗi 32-bit/64-bit hoặc quyền truy cập
                        // Dự phòng sang kiểm tra chung
                    }

                    if (hasDx12)
                    {
                        Logger.Info($"Process PID {proc.Id} has loaded d3d12.dll (DirectX 12 Active).");
                    }
                    else
                    {
