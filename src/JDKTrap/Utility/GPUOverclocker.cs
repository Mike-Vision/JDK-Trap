...
public sealed class AggressivePerformanceManager : IDisposable
{
    ...
    private SharpDX.Direct3D11.Buffer? _gpuBuffer;
    private UnorderedAccessView? _gpuUav;
    ...
    private void InitializeGpuResources()
    {
        try
        {
            ...
            using (var initialData = new DataStream(elementCount * sizeof(uint), true, true))
            {
                for (int i = 0; i < elementCount; i++) initialData.Write((uint)i);
                initialData.Position = 0;

                _gpuBuffer = new SharpDX.Direct3D11.Buffer(_d3dDevice, initialData, bufferDesc);
                ...
                _gpuUav = new UnorderedAccessView(_d3dDevice, _gpuBuffer, uavDesc);
    ...
    private void TryDispatchGpuWork(int multiplier = 1)
    {
        try
        {
            if (!_gpuResourcesInitialized || _d3dDevice == null || _computeShader == null || _gpuUav == null) return;

            var ctx = _d3dDevice.ImmediateContext;

            ctx.ComputeShader.Set(_computeShader);
            ctx.ComputeShader.SetUnorderedAccessView(0, _gpuUav);
