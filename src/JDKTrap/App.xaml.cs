...
using JDKTrap.UI.Elements.Bootstrapper;
        ...
        public static void AssertWindowsOSVersion()
        {
            ...
            if (major < 7)
            {
                ...
                if (!LaunchSettings.QuietFlag.Active)
                    Frontend.ShowMessageBox("Your Windows Version is not supported with JDKTrap!", MessageBoxImage.Error);
            ...
            Task.Run(async () =>
            {
                while (!token.IsCancellationRequested)
                {
                    try
                    {
                        ...
                        {
                            // Tự động tối ưu hoá bộ nhớ đệm luồng xử lý nhẹ nhàng
                            // CLR sẽ tự giải phóng tốt hơn là ép dọn định kỳ liên tục
                        }
                    }
                    catch (Exception ex)
                    {
                    ...
                    await Task.Delay(TimeSpan.FromMinutes(10), token); // Chuyển sang chu kỳ 10 phút để tránh giật lag
                }
            }, token);
        }
