...
namespace JDKTrap.RobloxInterfaces
{
    public static class Deployment
    {
        ...
        static Deployment()
        {
            SharedHttp.DefaultRequestHeaders.TryAddWithoutValidation("User-Agent", "JDKTrapUpdater/2.0");
        ...
        private static async Task<T> SafeGetJson<T>(string url)
        {
            ...
            for (int attempt = 1; attempt <= retries; attempt++)
            {
                ...
                catch (Exception ex) when ((ex is HttpRequestException || ex is TaskCanceledException || ex is System.IO.IOException) && attempt < retries)
                {
                    App.Logger.WriteLine("Deployment::SafeGetJson", $"Transient error ({attempt}/{retries}) fetching {url}: {ex.Message}. Retrying in {delayMs}ms...");
            ...
            DownloadForModGenerator(bool overwrite = false)
        {
            ...
            string tmp = Path.Combine(Path.GetTempPath(), "JDKTrap");
