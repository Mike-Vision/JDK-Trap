...
using Strings = JDKTrap.Resources.Strings;
...
using static JDKTrap.UI.ViewModels.Settings.ModsViewModel;
...
namespace JDKTrap
{
    public class Bootstrapper
    {
        ...
        public async Task Run()
        {
            ...
            try
            {
                using (Mutex.OpenExisting("JDKTrap-Bootstrapper"))
                {
                    App.Logger.WriteLine(LOG_IDENT, "JDKTrap-Bootstrapper mutex exists, waiting...");
            ...
            await using var mutex = new AsyncMutex(false, "JDKTrap-Bootstrapper");
        ...
        private async Task CheckAndApplyUpdate(string logIdent)
        {
            ...
            if (ok)
            {
                App.Logger.WriteLine(logIdent, "Update installed restarting JDKTrap...");
                RestartApplication();
            }
            else
            {
        ...
        private async Task GetLatestVersionInfo()
        {
            ...
            using var response = await WithRetryAsync(async () =>
            {
                return await App.HttpClient.GetAsync(
            ...
            using var manifestResp = await WithRetryAsync(async () =>
            {
                return await App.HttpClient.GetAsync(
        ...
        private void RestartMemoryCleanerFromSettings()
        {
            ...
            var settings = JDKTrapRobloxSettingsManager.Load();
        ...
        public static async Task ApplySkyboxPatchToRobloxStorageAsync()
        {
            ...
            foreach (var (hash, folder) in SkyboxPatchFolderMap)
            {
                ...
                try
                {
                    byte[] data = await WithRetryAsync(async () =>
                    {
                        return await http.GetByteArrayAsync(githubBase + hash);
                    }, "ApplySkyboxPatchToRobloxStorage");
                    if (File.Exists(dest))
